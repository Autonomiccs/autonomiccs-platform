...
The Autonomiccs platform is an open source software that provides the vital components needed to manage a cloud computing environment autonomously; we act at the infrastructure as a service level by providing a plugin to <a href="https://github.com/apache/cloudstack">Apache CloudStack</a>.

Our mission is to take the orchestration of cloud computing infrastructures to the next level. To achieve that, we have developed a distributed and autonomic plugin that can monitor a CC environment and take decisions in order to fulfill the goals of said environment. We do not wish to remove the need for human administrators, but to improve their work. Our mission involves optimizing and making CC environments more efficient and stable without the need to rely on human administrators.
